...
const PendientesRegistroModal: React.FC<PendientesRegistroModalProps> = ({
  ...
  const annotated = useMemo(() => {
    ...
    const nameSet = new Set(existingArr.map((d: any) => String(d.nombre_archivo || d.nombre_original || d.name || d.title || '').trim()).filter(Boolean));
    ...
    return items.map((f) => {
      ...
      const isRegistered = Boolean((fileId && driveIdSet.has(fileId)) || (filename && nameSet.has(filename)));
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-500">
                          ...
                          {a.isRegistered ? <span className="ml-2 px-2 py-0.5 bg-green-100 text-green-800 rounded text-xs">Registrado</span> : <span className="ml-2 px-2 py-0.5 bg-amber-100 text-amber-800 rounded text-xs">Pendiente</span>}
                        </div>
                      </div>
                    </div>
