...
import {
  Prerrequisito,
  useAllPrerrequisitos,
  useCreatePrerrequisito,
  useDeletePrerrequisito,
  usePrerrequisitosByCliente,
  useUpdatePrerrequisito,
} from '../../hooks/useGestionPrerrequisitos';
...
export const PrerrequisitosCliente: React.FC<PrerrequisitosClienteProps> = ({
  clienteId,
}) => {
  const [newPrerrequisito, setNewPrerrequisito] = useState({
    tipo_documento: '',
    dias_duracion: '',
  });
  ...
  const [selectedClienteForCreate, setSelectedClienteForCreate] = useState<
    number | null
  >(clienteId ?? null);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editingData, setEditingData] = useState<{
    tipo_documento: string;
    dias_duracion: string | number;
  }>({ tipo_documento: '', dias_duracion: '' });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        ).default.getClientes({ limit: 2000 });
        if (!mounted) return;
        ...
        setClients(
          list.map((c: any) => ({
            id: c.id,
            nombre: c.nombre || c.nombre_completo || `Cliente ${c.id}`,
          }))
        );
      } catch (e) {
  ...
  const { data: prerrequisitosCliente, isLoading: isLoadingCliente } =
    usePrerrequisitosByCliente(clienteId, {
      enabled: !isGlobalMode,
    });

  const { data: prerrequisitosGlobales, isLoading: isLoadingGlobales } =
    useAllPrerrequisitos({
      enabled: true,
    });
  ...
  const prerrequisitosMostrados = useMemo(() => {
    ...
    if (isGlobalMode) {
      ...
      const tiposDeDocumentoCliente = new Set(
        cliente.map(p => p.tipo_documento)
      );
      const globalesFiltrados = globales.filter(
        pGlobal => !tiposDeDocumentoCliente.has(pGlobal.tipo_documento)
      );
    ...
    const filtered = searchTerm
      ? combinedList.filter(p =>
          p.tipo_documento.toLowerCase().includes(searchTerm.toLowerCase())
        )
      : combinedList;
  ...
  React.useEffect(() => {
    try {
      ...
      console.debug(
        'PrerrequisitosCliente - clienteId:',
        clienteId,
        'prerrequisitosCliente:',
        prerrequisitosCliente,
  ...
  const handleCreate = () => {
    ...
    const payload = {
      ...newPrerrequisito,
      cliente_id:
        typeof selectedClienteForCreate !== 'undefined'
          ? selectedClienteForCreate
          : clienteId,
      dias_duracion: newPrerrequisito.dias_duracion
        ? Number(newPrerrequisito.dias_duracion)
        : null,
    };
  ...
  const handleDelete = (id: number) => {
    if (
      window.confirm(
        '¿Estás seguro de que quieres eliminar este prerrequisito?'
      )
    ) {
      deleteMutation.mutate(
        { id, cliente_id: clienteId },
        {
  ...
  const handleUpdate = () => {
    ...
    const payload = {
      ...
      data: {
        ...
        dias_duracion: editingData.dias_duracion
          ? Number(editingData.dias_duracion)
          : null,
      },
  ...
  const allPrerrequisitosTipos = useMemo(
    () =>
      Array.from(
        new Set(prerrequisitosGlobales?.map(p => p.tipo_documento) ?? [])
      ),
    [prerrequisitosGlobales]
  );
      ...
      <div className='max-w-4xl mx-auto'>
        <div className='bg-white p-4 rounded-lg shadow-sm mb-4'>
          <h3 className='text-lg font-semibold text-gray-800 mb-2'>
            {isGlobalMode
              ? 'Crear Prerrequisito Global'
              : 'Añadir Prerrequisito para Cliente'}
          </h3>
          <div className='grid grid-cols-1 md:grid-cols-3 gap-4 items-end'>
            <div className='md:col-span-2'>
              ...
              {!isGlobalMode && (
                <div className='mt-2'>
                  <label className='block text-xs text-gray-600'>
                    Asociar a cliente (parciales)
                  </label>
                  <select
                    value={selectedClienteForCreate ?? ''}
                    onChange={e =>
                      setSelectedClienteForCreate(
                  ...
                  >
                    <option value=''>
                      Seleccionar cliente (por defecto el cliente actual)
                    </option>
                    {clients.map(c => (
              ...
              <datalist id='prerrequisitos-tipos'>
                {allPrerrequisitosTipos.map(tipo => (
        ...
        <div className='mt-6'>
          <h3 className='text-xl font-semibold text-gray-800 mb-4'>
            {isGlobalMode
              ...
              : 'Lista de Prerrequisitos del Cliente'}
          </h3>
          <div className='mb-4'>
            <input
              type='text'
          ...
          <ul className='space-y-2'>
            {prerrequisitosMostrados.map(prerrequisito => {
              const isGlobalInClientView =
                !isGlobalMode && prerrequisito.es_global;
                    ...
                    <>
                      <div className='flex-1 flex items-center'>
                        ...
                        <div>
                          <p className='font-medium text-gray-800'>
                            {prerrequisito.tipo_documento}
                          </p>
                          <p className='text-sm text-gray-500'>
                            {prerrequisito.dias_duracion
                              ? `Válido por ${prerrequisito.dias_duracion} días`
                      ...
                      <div className='flex items-center space-x-2'>
                        {!isGlobalInClientView && (
                          <>
                            <button
                              onClick={() => startEditing(prerrequisito)}
                              className='text-blue-500 hover:text-blue-700 p-1'
                            >
                              <Edit size={18} />
                            </button>
                            <button
                              onClick={() => handleDelete(prerrequisito.id)}
          ...
          {/* Acción inferior para crear prerrequisito global directamente, según especificación */}
          <div className='mt-6 p-4 bg-white rounded-lg shadow-sm border'>
            <h4 className='text-lg font-semibold text-gray-800 mb-2'>
              Crear Prerrequisito Global
            </h4>
            <p className='text-sm text-gray-600 mb-4'>
              Endpoint: <code>POST /api/prerrequisitos</code> (sin{' '}
              <code>cliente_id</code> en el body → global). Campos:{' '}
              <code>tipo_documento</code> (requerido), <code>descripcion</code>{' '}
              (opcional), <code>dias_duracion</code> (opcional, entero ≥ 0).
            </p>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4 items-end'>
              <div className='md:col-span-1'>
                ...
                <input
                  ...
                  className='mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm'
                  placeholder='Ej: licencia_conducir'
                />
              </div>
              <div className='md:col-span-1'>
                <label className='block text-sm font-medium text-gray-700'>
                ...
                <input
                  type='text'
                  onChange={e => {
                    // Mantener descripción en estado aparte usando editingData para no romper el shape original
                    setEditingData({
                      ...editingData,
                      tipo_documento: newPrerrequisito.tipo_documento,
                      dias_duracion: newPrerrequisito.dias_duracion,
                    });
                    // Usaremos editingData.descripcion al enviar
                    (editingData as any).descripcion = e.target.value;
                  }}
                  className='mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm'
              ...
              <div className='md:col-span-3 text-right'>
                <button
                  onClick={() => {
                    ...
                    const desc = (editingData as any).descripcion;
                    if (desc && String(desc).trim().length > 0)
                      payload.descripcion = String(desc).trim();
                    // Global: NO incluir cliente_id
                    createMutation.mutate(payload, {
                      onSuccess: () => {
                        toast.success('Prerrequisito global creado.');
                        setNewPrerrequisito({
                          tipo_documento: '',
                          dias_duracion: '',
                        });
                      ...
                      onError: error =>
                        toast.error(`Error al crear global: ${error.message}`),
                    });
                  }}
                  disabled={createMutation.isLoading}
                ...
                >
                  <FaPlus className='mr-2' /> Crear Global
                </button>
              </div>
            </div>
          </div>
